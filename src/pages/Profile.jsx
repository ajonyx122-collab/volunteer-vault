...
export default function Profile() {
  const totalCauseHours = currentUser.causes.reduce((sum, c) => sum + c.hours, 0)
  const nextBadge = currentUser.badges.find((b) => !b.earned)
    ...
    <div>
      ...
      <section className="bg-brand-green">
        <div className="mx-auto flex max-w-5xl flex-col items-center gap-4 px-4 py-12 text-center sm:px-6">
          ...
          <div>
            <h1 className="font-display text-2xl font-extrabold text-cream-text">{currentUser.displayName}</h1>
            <p className="text-cream-muted">
              {currentUser.school} · Class of {currentUser.gradYear}
            </p>
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            <button className="rounded-pill bg-gold px-5 py-2 text-sm font-bold text-gold-text shadow-soft transition-transform hover:scale-105">
              Share my vault
            </button>
            <button className="rounded-pill border border-cream-muted px-5 py-2 text-sm font-bold text-cream-text transition-colors hover:bg-brand-green-light">
              Certificate
            </button>
          </div>
          <p className="text-xs text-cream-muted">volunteervault.org/{currentUser.username}</p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard value={currentUser.verifiedHours} label="Verified hours" />
          <StatCard value={`${currentUser.streakWeeks}wk`} label="Streak" />
          <StatCard value={currentUser.badges.filter((b) => b.earned).length} label="Badges" />
          <StatCard value={currentUser.causes.length} label="Causes" />
        </div>
